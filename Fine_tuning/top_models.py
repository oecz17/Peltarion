...
def eval_model(model, data_loader, device, best_acc, writer, step_t, metrics=False):
    ...
    with torch.no_grad():
        ...
        if accuracy>best_acc:
            best_acc = accuracy
            if not args.multi:
                torch.save(model.state_dict(), "/workspace/models/"+ args.model + "_ft.pt")           
...
writer = SummaryWriter('/workspace/logs/' + args.model)
...
if args.eval:
    model.load_state_dict(torch.load("/workspace/models/"+ args.model + "_ft.pt"))
...
if args.continue_training:
    model.load_state_dict(torch.load("/workspace/models/"+ args.model + ".pt"))
    args.warmup_steps = 0
