...
def eval_model(model, data_loader, device, best_acc, writer, step_t, metrics=False):
    ...
    with torch.no_grad():
        ...
        if accuracy>best_acc:
            best_acc = accuracy
            if not args.multi:
                torch.save(model.state_dict(), "/workspace/models/mt5.pt")  
...
df = pd.read_csv("/workspace/models/seq_dataset.csv")
...
writer = SummaryWriter('/workspace/logs/mt5_lr')
...
if args.eval:
    model.load_state_dict(torch.load("/workspace/models/mt5.pt"))
